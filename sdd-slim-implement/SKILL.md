...
description: |
  ...
  然后重新读取 feature artifact set；实现阶段必须把每个未完成 `T*` 作为一个独立实现包先交给 subagent 执行，`P*` 只保留为来源点与主 agent 的聚合维度；默认串行逐个处理。若检测到 `--mutiAgent` 或用户明确要求多个 agent 并行实现，主 agent 必须先做独立性判断：能并行就直接开启，不能并行就自动降级为串行并记录原因。主 agent 负责分派、审核、逐条回写 `worklog.md` 中的 checklist / execution notes、同步 `spec.md` 状态、重新计算剩余未完成 `T*` 与派生的 `P*` 完成态；只有全部未完成 `T*` 均已实现完毕，且所有 `P*` 已被合法聚合到终态，或出现无法安全继续的 blocker 时才能停止。
