...
description: |
  ...
  该 skill 会先归档参考项目与复刻目标，再生成 learning 版 `.sdd-slim/<feature-name>.spec.md`；重点是先把整个项目拆成多个可学习模块，为每个模块规划知识点、学习顺序与 task，而不是按纯技术层横切实现。
...
1. 读取 `specify.md`
2. 使用 `templates/learning-spec.md` 作为 canonical learning spec 模板
3. 模块级代码研究 subagent prompt 参考 `prompts/module-subagent-task-prompt.md`
4. 模块确认提问参考 `prompts/module-confirmation-question.md`
5. 示例见 `examples/example.spec.md`

## 独立性规则

- 本 skill 只负责生成 / 更新 `.sdd-slim/<feature-name>.requirement.md` 与 `.sdd-slim/<feature-name>.spec.md`
- 不写任何产品代码
