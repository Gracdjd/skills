...
description: |
  ...
  默认分支名格式为 `YYYY.MM.DD.<task-name>`；任务名优先来自用户输入，其次可来自当前 spec 名称，仍不明确时再用 `askquestion` 澄清。
...
1. 读取 `submit.md`
2. 如果任务名不明确，优先从当前 `.sdd-slim/*.spec.md`、当前用户请求或当前任务上下文提取；仍不明确时使用 `askquestion`
...
- 不自动 push
- 不自动创建 PR
- 不修改 `sdd-slim-plan` / `sdd-slim-implement` / `sdd-slim-review` / `sdd-slim-fix` 的任何规则
