...
description: |
  ...
  review 阶段中的 final verification harness 也属于同一闭环：web / browser UI 用 `e2e`，其他目标用 `unit`，并必须在 harness 前先把 plan 阶段定义的测试设计落成真实 unit / e2e 测试文件，最终产出包含 unit coverage、e2e success rate 与 project regression 结果的统一测试报告。
  对 web / browser UI，agent 侧默认使用 Playwright MCP 浏览器工具链执行自动化；仓库内 Playwright 测试文件用于长期回归资产沉淀，项目命令只作为 CI 或显式复跑的次级路径。
